    ...
    <RulesetProvider rulesetId={rulesetId}>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', paddingTop: 'var(--dev-banner-h, 0px)' }}>
        ...
        <button
          ...
          className="fixed bottom-6 right-6 z-40 rounded-full bg-amber-200/10 border border-amber-200/20 text-amber-200/50 hover:bg-amber-200/20 hover:text-amber-200/80 hover:border-amber-200/40 transition-all shadow-lg backdrop-blur-sm"
          style={{ width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
          title={shortcutHintsSeen ? "Quick Journal Note (Ctrl+N)" : "Quick Journal Note"}
        >
          <PenLine size={16} />
        </button>
        ...
        {showQuickJournal && (
          <div
            ...
            onClick={(e) => { if (e.target === e.currentTarget) setShowQuickJournal(false); }}
          >
            <div
              className="rounded-xl border border-amber-200/15 shadow-2xl"
              style={{ background: 'rgba(12,10,20,0.95)', backdropFilter: 'blur(20px)', width: 360, maxHeight: '70vh', display: 'flex', flexDirection: 'column' }}
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-amber-200/10">
                <div className="flex items-center gap-2 text-amber-200/70 text-sm font-display">
                  <PenLine size={14} />
                  Quick Journal
                </div>
                <button
              ...
              {/* Body */}
              <div className="px-4 py-3 space-y-3" style={{ overflowY: 'auto' }}>
                <input
                  type="text"
                  ...
                  onKeyDown={(e) => { if (e.key === 'Escape') setShowQuickJournal(false); if (e.ctrlKey && e.key === 'Enter') saveQuickJournal(); }}
                  autoFocus
                />
                <textarea
                  className="input w-full text-sm"
                  ...
                  rows={5}
                  value={quickJournalBody}
                  onChange={(e) => setQuickJournalBody(e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Escape') setShowQuickJournal(false); if (e.ctrlKey && e.key === 'Enter') saveQuickJournal(); }}
              ...
              {/* Footer */}
              <div className="flex items-center justify-between px-4 py-3 border-t border-amber-200/10">
                <span className="text-[10px] text-amber-200/25">Ctrl+Enter to save</span>
                <button
                  onClick={saveQuickJournal}
                  disabled={quickJournalSaving || !quickJournalTitle.trim()}
                  className="btn-primary text-xs px-4 py-1.5"
                  style={{ opacity: (!quickJournalTitle.trim() || quickJournalSaving) ? 0.4 : 1 }}
                >
                  {quickJournalSaving ? 'Saving...' : 'Save'}
                </button>
              </div>
            </div>
          </div>
