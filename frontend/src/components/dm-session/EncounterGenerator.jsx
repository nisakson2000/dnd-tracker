...
import { XP_THRESHOLDS as XP_THRESHOLDS_RAW, CR_XP as CR_XP_RAW } from '../../utils/encounterMath';

/* ─── Adapt canonical array-based XP_THRESHOLDS to named-key format ─── */
const DIFFICULTY_KEYS = ['easy', 'medium', 'hard', 'deadly'];
const XP_THRESHOLDS = Object.fromEntries(
  Object.entries(XP_THRESHOLDS_RAW).map(([level, arr]) => [
    level,
    Object.fromEntries(DIFFICULTY_KEYS.map((key, i) => [key, arr[i]])),
  ])
);

/* ─── Adapt canonical string-keyed CR_XP to numeric keys for monster tables ─── */
const CR_XP = {};
for (const [cr, xp] of Object.entries(CR_XP_RAW)) {
  if (cr === '1/8') CR_XP[0.125] = xp;
  else if (cr === '1/4') CR_XP[0.25] = xp;
  else if (cr === '1/2') CR_XP[0.5] = xp;
  else CR_XP[Number(cr)] = xp;
}
