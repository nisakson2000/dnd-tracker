...
pub fn get_lore_notes(
    ...
    state.with_char_conn(&character_id, |conn| {
        let mut stmt = conn
            .prepare("SELECT id, title, category, body, created_at, updated_at FROM lore_notes ORDER BY updated_at DESC")
...
pub fn add_lore_note(
    ...
    state.with_char_conn(&character_id, |conn| {
        ...
        conn.execute(
            ...
            rusqlite::params![payload.title, payload.category, payload.body, now, now],
        )
        .map_err(|e| e.to_string())?;
...
pub fn update_lore_note(
    ...
    state.with_char_conn(&character_id, |conn| {
        ...
        let updated = conn
            .execute(
                "UPDATE lore_notes SET title=?1, category=?2, body=?3, updated_at=?4 WHERE id=?5",
                rusqlite::params![payload.title, payload.category, payload.body, now, note_id],
            )
            .map_err(|e| e.to_string())?;
        if updated == 0 {
