...
pub fn create_campaign_npc(
    ...
    with_campaign_conn(&state, |conn| {
        conn.execute(
            "INSERT INTO campaign_npcs (id, campaign_id, name, role, race, location, description, dm_notes, visibility, known_info_json, status, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9, '[]', 'alive', ?10)",
            params![id, campaign_id, name, role, race, location, description, dm_notes, visibility, now],
...
pub fn list_campaign_npcs(
    ...
    with_campaign_conn(&state, |conn| {
        let mut stmt = conn.prepare(
            "SELECT id, name, role, race, location, description, dm_notes, visibility, known_info_json, status, created_at FROM campaign_npcs WHERE campaign_id = ?1 ORDER BY name"
...
pub fn list_campaign_npcs_player(
    ...
    with_campaign_conn(&state, |conn| {
        let mut stmt = conn.prepare(
            "SELECT id, name, role, race, location, description, visibility, known_info_json, status FROM campaign_npcs WHERE campaign_id = ?1 AND visibility != 'dm_only' ORDER BY name"
        ).map_err(|e| format!("Failed to query campaign NPCs: {}", e))?;
...
pub fn update_campaign_npc(
    ...
    with_campaign_conn(&state, |conn| {
        let rows = conn.execute(
            "UPDATE campaign_npcs SET name = ?1, role = ?2, race = ?3, location = ?4, description = ?5, dm_notes = ?6, visibility = ?7 WHERE id = ?8",
            params![name, role, race, location, description, dm_notes, visibility, npc_id],
